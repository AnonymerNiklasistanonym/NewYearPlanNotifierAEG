...
DIR_OF_SCRIPT = os.path.join(HOME_DIR,
                             os.path.join("Documents/GitHubBeta", "NewYearPlanNotifierAEG"))
# Documents
...
try:
    ...
    HTTP = httplib2.Http()
    HEADERS = {'Cookie': "6024a7383aa153ec184c3ea3382596a7=fli24m6oth1a2p43d8a7d64vs7"}
    ...
    SOUP_HTML_DATA = SOUP_HTML_DATA.replace("href=\"/", "href=\"http://www.aeg-boeblingen.de/")
    ...
    if JSON_DATA_SAVED_FILE is None or JSON_DATA_WEBSITE != JSON_DATA_SAVED_FILE:
        ...
        if USE_GMAIL:
            ...
            NON_ASCII_RE = re.compile(u'([\x00-\x7f]+)|([^\x00-\x7f])', re.UNICODE).sub
            ...
            for recipient in RECIPIENTS:
                GMAIL_SERVER.send_html(recipient, "Neuer Stundenplan?", EMAIL_TEXT)
